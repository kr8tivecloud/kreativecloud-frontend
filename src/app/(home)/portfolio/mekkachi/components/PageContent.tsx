import { ServicePills } from "@/components/home/portfolio/ServicePills";
import React from "react";
import {
  GalleryImage,
  PortfolioGallery,
} from "../../../../../components/home/portfolio/PortfolioGallery";
import {
  MekkachItem1,
  MekkachItem2,
  MekkachItem3,
  MekkachItem4,
  MekkachItem5,
  MekkachItem6,
} from "../../../../../../public/images";

const galleryImages: GalleryImage[] = [
  {
    src: MekkachItem1,
    alt: "Exhibition 1",
    cols: 6,
  },
  {
    src: MekkachItem2,
    alt: "Exhibition 2",
    cols: 2,
  },
  {
    src: MekkachItem3,
    alt: "Exhibition 3",
    cols: 2,
  },
  {
    src: MekkachItem4,
    alt: "Exhibition 4",
    cols: 2,
  },
  {
    src: MekkachItem5,
    alt: "Exhibition 5",
    cols: 6,
  },
  {
    src: MekkachItem6,
    alt: "Exhibition 6",
    cols: 6,
  },
];

const services = ["Website", "UI/UX Design", "Branding", "Marketing"];
const link = "https://mekkachi.myshopify.com/";

export default function PageContent() {
  return (
    <div className="px-4 sm:container my-24">
      {/* TITLE */}
      <div>
        <h2 className="font-bold text-3xl">Mekkachi</h2>
        <ServicePills services={services} />
        <p className="max-w-[670px]">
          Mekkachi wanted a bold, stylish Shopify store to match their clothing
          brand’s energy, so we crafted a sleek, high-converting ecommerce
          experience with all the right touches.
          <br />
          <a target="_blank" href={link} className="underline text-[#FE922A]">
            View website
          </a>
        </p>

        {/* PASSWORD */}
        <div className="mt-4 bg-[#0F0E14] border border-white/25 inline-block px-4 py-2 rounded-full text-sm">
          Password: <span className="text-white font-bold">Kreativecloud</span>
        </div>
        {/* END PASSWORD */}
      </div>
      {/* END TITLE */}

      {/* PORTFOLIO GALLERY */}
      <PortfolioGallery images={galleryImages} />
      {/* END PORTFOLIO GALLERY */}
    </div>
  );
}
