import { ServicePills } from "@/components/home/portfolio/ServicePills";
import React from "react";
import {
  GalleryImage,
  PortfolioGallery,
} from "../../../../../components/home/portfolio/PortfolioGallery";
import {
  CafeItem1,
  CafeItem2,
  CafeItem3,
  CafeItem4,
} from "../../../../../../public/images";

const galleryImages: GalleryImage[] = [
  {
    src: CafeItem1,
    alt: "Exhibition 1",
    cols: 6,
  },
  {
    src: CafeItem2,
    alt: "Exhibition 2",
    cols: 3,
  },
  {
    src: CafeItem3,
    alt: "Exhibition 3",
    cols: 3,
  },
  {
    src: CafeItem4,
    alt: "Exhibition 4",
    cols: 6,
  },
];

const services = ["Website", "UI/UX Design"];
const link = "https://cafeczen.myshopify.com/";

export default function PageContent() {
  return (
    <div className="px-4 sm:container my-24">
      {/* TITLE */}
      <div>
        <h2 className="font-bold text-3xl">Café</h2>
        <ServicePills services={services} />
        <p className="max-w-[670px]">
          Café needed a digital space as refined as their fashion line—so we
          designed a modern, immersive Shopify store that blends clean visuals
          with a seamless shopping experience.
          <br />
          <a target="_blank" href={link} className="underline text-[#FE922A]">
            {link}
          </a>
        </p>

        {/* PASSWORD */}
        <div className="mt-4 bg-[#0F0E14] border border-white/25 inline-block px-4 py-2 rounded-full text-sm">
          Password: <span className="text-white font-bold">open</span>
        </div>
        {/* END PASSWORD */}
      </div>
      {/* END TITLE */}

      {/* PORTFOLIO GALLERY */}
      <PortfolioGallery images={galleryImages} />
      {/* END PORTFOLIO GALLERY */}
    </div>
  );
}
